<?php
/*
Plugin Name: Woostroid Theme Wizard
Plugin URI:
Description: Woostroid theme installation wizard.
Author: Zemez
Author URI:
Version: 1.0.0
Text Domain: ws-theme-wizard
...
if ( ! class_exists( 'WS_Theme_sWizard' ) ) {

	/**
	 * Define WS_Theme_Wizard class
	 */
	class WS_Theme_Wizard {
		 ...
		 * @var string
		 */
		private $slug = 'ws-theme-wizard';

		/**
		 * Nonce action name.
		 * @var string
		 */
		public $_nonce = 'ws-theme-wizard-nonce';
		 ...
		 * @var string
		 */
		public $prefix = 'ws_theme_wizard_';
		...
		function __construct() {
			...
			$this->settings = array(
				'options' => array(
					'parent_data' => 'ws-theme-wizard-installed-parent',
					'child_data'  => 'ws-theme-wizard-installed-child',
				),
			);
		...
		public function hooks() {
			...
			add_action( 'admin_menu', array( ws_theme_interface(), 'register_page' ) );
			add_action( 'admin_menu', array( ws_theme_interface(), 'register_page' ) );
		}
		...
		public function lang() {
			load_plugin_textdomain( 'ws-theme-wizard', false, $this->path( 'languages' ) );
		}
		...
		public function activation_redirect() {
			...
			$enbled = apply_filters( 'ws-theme-wizard/activation-redirect-enabled', $enbled );
			...
			delete_transient( $this->slug() . '_redirect' );
			wp_redirect( ws_theme_interface()->get_page_link() );
			die();
		}
		...
		public function enqueue_assets( $hook ) {
			...
			wp_localize_script( $this->slug(), 'JetThemeWizardSettings', array(
				...
				'errors' => array(
					'empty' => esc_html__( '* Please, fill this field', 'ws-theme-wizard' ),
				),
			) );
			 ...
			 */
			do_action( 'ws-theme-wizard/enqueue-assets' );
		}
...
/**
 * Returns instance of WS_Theme_Wizard
 *
 * @return object
 */
function ws_theme_wizard() {
	return WS_Theme_Wizard::get_instance();
}

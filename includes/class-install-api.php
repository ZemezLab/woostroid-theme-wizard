...
if ( ! class_exists( 'WS_Theme_Wizard_Install_API' ) ) {

	/**
	 * Define WS_Theme_Wizard_Install_API class
	 */
	class WS_Theme_Wizard_Install_API {
		...
		public function do_theme_install() {
			...
			$data    = array();
			$success = true;
			$message = esc_html__( 'The theme is succesfully installed. Activating...', 'ws-theme-wizard' );
			...
			if ( is_wp_error( $result ) ) {
				...
				if ( ! isset( $skin->result->errors['folder_exists'] ) ) {
					...
					$message = esc_html__( 'The theme has been already installed. Activating...', 'ws-theme-wizard' );
				}
				...
				$message = esc_html__( 'Unable to connect to the filesystem. Please confirm your credentials.', 'ws-theme-wizard' );
		...
		public function do_plugin_install( $slug = null ) {
			...
			$data    = array();
			$success = true;
			$message = esc_html__( 'Plugins wizard installed and activated. Redirecting...', 'ws-theme-wizard' );
		...
		public function adjust_plugin_dir( $source, $remote_source, $upgrader ) {
			...
			if ( empty( $_REQUEST['action'] ) && 'ws_theme_wizard_install_plugins_wizard' !== $_REQUEST['action'] ) {
				return $source;
			}
		...
		public function adjust_theme_dir( $source, $remote_source, $upgrader ) {
			...
			// Ensure that is Wizard installation request
			if ( empty( $_REQUEST['action'] ) && 'ws_theme_wizard_install_parent' !== $_REQUEST['action'] ) {
				return $source;
			}
			...
			if ( true === $wp_filesystem->move( $from_path, $to_path ) ) {
				 ...
				 */
				do_action( 'ws-theme-wizard/source-rename-done', $theme_data );
				...
				return new WP_Error(
					'rename_failed',
					esc_html__( 'The remote plugin package does not contain a folder with the desired slug and renaming did not work.', 'ws-theme-wizard' ),
...
/**
 * Returns instance of WS_Theme_Wizard_Install_API
 *
 * @return object
 */
function ws_theme_install_api( $url = null ) {
	return WS_Theme_Wizard_Install_API::get_instance( $url );
}

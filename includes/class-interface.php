...
if ( ! class_exists( 'WS_Theme_Wizard_Interface' ) ) {

	/**
	 * Define WS_Theme_Wizard_Interface class
	 */
	class WS_Theme_Wizard_Interface {
		...
		public function get_page_link( $step = 'verification' ) {
			...
			return add_query_arg(
				array(
					'page' => esc_attr( ws_theme_wizard()->slug() ),
					'step' => esc_attr( $step ),
				),
				$base
			);
		...
		public function get_template( $template, $data = false ) {
			...
			$file = locate_template( ws_theme_wizard()->slug() . '/' . $template );

			if ( ! $file ) {
				$file = ws_theme_wizard()->path( 'templates/' . $template );
			}

			$file = apply_filters( 'ws-theme-wizard/template-path', $file, $template );
...
/**
 * Returns instance of WS_Theme_Wizard_Interface
 *
 * @return object
 */
function ws_theme_interface() {
	return WS_Theme_Wizard_Interface::get_instance();
}
